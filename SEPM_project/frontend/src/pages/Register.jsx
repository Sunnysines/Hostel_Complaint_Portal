...
export default function Register() {
  ...
  const handleRegister = async (e) => {
    ...
    if (!netid || !email || !password || !confirmPassword) {
    ...
    try {
      setLoading(true);
      await authService.register({ netid, email, password, role });
      ...
      <div className="max-w-5xl w-full bg-white dark:bg-slate-900 rounded-xl overflow-hidden login-card-shadow border border-gray-100 dark:border-slate-800" style={{ boxShadow: '0 10px 25px -5px rgba(0, 0, 0, 0.1), 0 8px 10px -6px rgba(0, 0, 0, 0.1)' }}>
        ...
        <div className="flex flex-col md:flex-row">
          ...
          <div className="flex-1 p-10 bg-gray-50 dark:bg-slate-800/50">
            <h2 className="text-2xl font-bold text-slate-800 dark:text-white mb-4">Account Registration</h2>
            <div className="space-y-4 text-slate-600 dark:text-slate-300 leading-relaxed">
              <p className="font-medium text-lg">Join the Student Hostel Complaint Portal.</p>
              <p>Create an account to report maintenance issues, track the status of your complaints, and view official responses from the hostel administration office.</p>
              
              <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border-l-4 border-primary mt-6">
                <p className="text-sm">
                  <span className="font-bold block mb-1">Registration Instructions:</span>
                  Please register using your official NetID and University Email.
                  <br/>(e.g., If your email is <span className="font-mono text-primary">abcd@university.edu.in</span>, your NetID is <span className="font-mono font-bold">abcd</span>).
                </p>
              </div>
            </div>
          </div>
          ...
          {/* Form Side */}
          <div className="w-full md:w-[450px] p-10 border-l border-gray-100 dark:border-slate-800">
            <div className="mb-6">
              <h3 className="text-xl font-bold text-primary mb-1">Sign Up</h3>
              <div className="h-1 w-12 bg-primary rounded"></div>
            </div>
            
            {error && (
              <div className="mb-4 bg-red-50 dark:bg-red-900/20 border-l-4 border-red-500 p-3 text-sm text-red-700 dark:text-red-400">
                {error}
              </div>
            )}
            ...
            <form className="space-y-4" onSubmit={handleRegister}>
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <input 
                    className="block w-full pl-10 pr-3 py-2 bg-white dark:bg-slate-900 border border-gray-300 dark:border-slate-700 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary sm:text-sm dark:text-white transition-all" 
                    id="email" 
                    placeholder="abcd@university.edu.in" 
                    type="email"
                    value={email}
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <span className="material-icons text-gray-400 text-sm">lock_outline</span>
                  </div>
                  <input 
                    className="block w-full pl-10 pr-3 py-2 bg-white dark:bg-slate-900 border border-gray-300 dark:border-slate-700 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary sm:text-sm dark:text-white transition-all" 
                    id="confirmPassword" 
              ...
              <div className="space-y-1">
                <label className="text-sm font-medium text-slate-700 dark:text-slate-300" htmlFor="role">
                  Account Type (Testing Only)
                </label>
                <select
                  id="role"
                  value={role}
                  onChange={(e) => setRole(e.target.value)}
                  className="block w-full px-3 py-2 bg-white dark:bg-slate-900 border border-gray-300 dark:border-slate-700 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary sm:text-sm dark:text-white transition-all"
                >
